...
from google.ads.googleads.v19.services.services.customer_client_link_service import (
    CustomerClientLinkServiceClient,
)
from google.ads.googleads.v19.services.services.customer_manager_link_service import (
...
from google.ads.googleads.v19.enums.types.manager_link_status import (
    ManagerLinkStatusEnum as ActualManagerLinkStatusEnumType,
)
...
class TestLinkManagerToClient(unittest.TestCase):
    ...
    @patch(
        "examples.account_management.link_manager_to_client.protobuf_helpers.field_mask"
    )
    @patch("examples.account_management.link_manager_to_client.GoogleAdsClient")
    def test_main_links_manager_and_client(
        self, mock_google_ads_client_class, mock_field_mask_helper
    ):
        # 1. Setup Mocks
        ...
        def get_type_side_effect(type_name):
            ...
            raise ValueError(
                f"Unexpected type requested by script: {type_name}"
            )
        ...
        mock_client_instance.enums.ManagerLinkStatusEnum = (
            ActualManagerLinkStatusEnumType.ManagerLinkStatus
        )
        ...
        mock_field_mask_helper.return_value = MagicMock(
            paths=["status"]
        )  # Simulating FieldMask(paths=['status'])
        ...
        expected_client_customer_resource_name = (
            f"customers/{client_customer_id}"
        )
        mock_ccl_service.customer_path.return_value = (
            expected_client_customer_resource_name
        )

        # Mock mutate_customer_client_link response
        mock_ccl_response = MagicMock()
        created_ccl_resource_name = (
            f"customers/{manager_customer_id}/customerClientLinks/link1"
        )
        mock_ccl_response.results = [
            MagicMock(resource_name=created_ccl_resource_name)
        ]
        mock_ccl_service.mutate_customer_client_link.return_value = (
            mock_ccl_response
        )
        ...
        mock_search_row.customer_client_link.manager_link_id = (
            manager_link_id_val
        )
        ...
        mock_search_response_container.result = iter(
            [mock_search_row]
        ...
        mock_cml_service.customer_manager_link_path.return_value = (
            expected_cml_resource_name
        )

        # Mock mutate_customer_manager_link response
        mock_cml_response = MagicMock()
        updated_cml_resource_name = (
            expected_cml_resource_name  # Usually same after update
        )
        mock_cml_response.results = [
            MagicMock(resource_name=updated_cml_resource_name)
        ]
        mock_cml_service.mutate_customer_manager_link.return_value = (
            mock_cml_response
        )
        ...
        link_manager_to_client.main(
            mock_client_instance, client_customer_id, manager_customer_id
        )
        ...
        mock_client_instance.get_service.assert_any_call(
        ...
        mock_client_instance.get_type.assert_any_call(
        ...
        mock_ccl_service.customer_path.assert_called_once_with(
            client_customer_id
        )
        ...
        self.assertEqual(
            ccl_call_args.kwargs["customer_id"], manager_customer_id
        )
        self.assertEqual(ccl_call_args.kwargs["operation"], mock_ccl_operation)
        self.assertEqual(
            mock_ccl_operation.create.client_customer,
            expected_client_customer_resource_name,
        )
        self.assertEqual(
            mock_ccl_operation.create.status,
            ActualManagerLinkStatusEnumType.ManagerLinkStatus.PENDING,
        )
        ...
        self.assertEqual(
            ga_search_call_args.kwargs["customer_id"], manager_customer_id
        )
        expected_query = f'''
        SELECT
        ...
        self.assertEqual(
            ga_search_call_args.kwargs["query"].strip(), expected_query.strip()
        )
        ...
        copy_from_args = mock_client_instance.copy_from.call_args[
            0
        ]  # Positional args
        self.assertEqual(
            copy_from_args[0], mock_cml_operation.update_mask
        )  # First arg is the target mask
        self.assertEqual(
            copy_from_args[1], mock_field_mask_helper.return_value
        )  # Second arg is the source mask
        ...
        self.assertEqual(
            cml_call_args.kwargs["customer_id"], client_customer_id
        )
        # operations is a list
        self.assertEqual(len(cml_call_args.kwargs["operations"]), 1)
        self.assertEqual(
            cml_call_args.kwargs["operations"][0], mock_cml_operation
        )
        self.assertEqual(
            mock_cml_operation.update.resource_name, expected_cml_resource_name
        )
        self.assertEqual(
            mock_cml_operation.update.status,
            ActualManagerLinkStatusEnumType.ManagerLinkStatus.ACTIVE,
        )
