...
def main(client, customer_id, hotel_center_account_id,
  ...
  # Add hotel campaign
  campaign_resource_name = add_hotel_campaign(client, customer_id,
...
def add_hotel_ad_group(client, customer_id, campaign_resource_name):
  ...
  ad_group.type = client.get_type('AdGroupTypeEnum', version='v2').HOTEL_ADS
...
def add_hotel_campaign(client, customer_id, budget_resource_name,
  ...
  campaign.percent_cpc.cpc_bid_ceiling_micros.value = (bid_ceiling_micro_amount)
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      ...
      type=int,
      required=True,
      help=('The bid ceiling micro amount for '
            'the hotel campaign.'))
  parser.add_argument(
      '-a',
      '--hotel_center_account_id',
