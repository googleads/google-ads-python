...
"""Accesses the google.ads.googleads.v2.services FeedItemTargetService API."""
...
from google.ads.google_ads.v2.services import feed_item_target_service_client_config
...
class FeedItemTargetServiceClient(object):
    ...
    _INTERFACE_NAME = 'google.ads.googleads.v2.services.FeedItemTargetService'
    ...
    def __init__(self, transport=None, channel=None, credentials=None,
        ...
        Args:
            transport (Union[~.FeedItemTargetServiceGrpcTransport,
                    Callable[[~.Credentials, type], ~.FeedItemTargetServiceGrpcTransport]): A transport
        ...
        if client_config is not None:
            ...
            client_config = feed_item_target_service_client_config.config
        ...
        if transport:
            if callable(transport):
                self.transport = transport(
                    ...
                    default_class=feed_item_target_service_grpc_transport.FeedItemTargetServiceGrpcTransport,
                )
            else:
                if credentials:
                    raise ValueError(
            ...
            self.transport = feed_item_target_service_grpc_transport.FeedItemTargetServiceGrpcTransport(
    ...
    def get_feed_item_target(
        ...
        Args:
            resource_name (str): The resource name of the feed item targets to fetch.
        ...
        Returns:
            A :class:`~google.ads.googleads_v2.types.FeedItemTarget` instance.
        ...
        if 'get_feed_item_target' not in self._inner_api_calls:
            self._inner_api_calls['get_feed_item_target'] = google.api_core.gapic_v1.method.wrap_method(
                self.transport.get_feed_item_target,
                default_retry=self._method_configs['GetFeedItemTarget'].retry,
                default_timeout=self._method_configs['GetFeedItemTarget'].timeout,
                client_info=self._client_info,
            )

        request = feed_item_target_service_pb2.GetFeedItemTargetRequest(
        ...
        return self._inner_api_calls['get_feed_item_target'](request, retry=retry, timeout=timeout, metadata=metadata)
    ...
    def mutate_feed_item_targets(
        ...
        Args:
            customer_id (str): The ID of the customer whose feed item targets are being modified.
            operations (list[Union[dict, ~google.ads.googleads_v2.types.FeedItemTargetOperation]]): The list of operations to perform on individual feed item targets.

                If a dict is provided, it must be of the same form as the protobuf
                message :class:`~google.ads.googleads_v2.types.FeedItemTargetOperation`
            retry (Optional[google.api_core.retry.Retry]):  A retry object used
                to retry requests. If ``None`` is specified, requests will not
                be retried.
            timeout (Optional[float]): The amount of time, in seconds, to wait
        ...
        Returns:
            A :class:`~google.ads.googleads_v2.types.MutateFeedItemTargetsResponse` instance.
        ...
        """
        # Wrap the transport method to add retry and timeout logic.
        if 'mutate_feed_item_targets' not in self._inner_api_calls:
            self._inner_api_calls['mutate_feed_item_targets'] = google.api_core.gapic_v1.method.wrap_method(
                self.transport.mutate_feed_item_targets,
                default_retry=self._method_configs['MutateFeedItemTargets'].retry,
                default_timeout=self._method_configs['MutateFeedItemTargets'].timeout,
                client_info=self._client_info,
            )

        request = feed_item_target_service_pb2.MutateFeedItemTargetsRequest(
        ...
        return self._inner_api_calls['mutate_feed_item_targets'](request, retry=retry, timeout=timeout, metadata=metadata)
