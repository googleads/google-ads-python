...
class TestCreateCustomer(unittest.TestCase):
    ...
    def test_main_creates_customer_correctly(
        self, mock_google_ads_client_class, mock_datetime
    ):
        # 1. Setup Mocks
        ...
        mock_customer_service.create_customer_client.return_value = (
        ...
        create_customer.main(mock_client_instance, manager_customer_id)
        ...
        customer_arg = kwargs.get(
            "customer_client"
        )  # Argument name is customer_client
        self.assertIsNotNone(customer_arg)
        self.assertEqual(
            customer_arg.descriptive_name, expected_descriptive_name
        )
        ...
        self.assertEqual(
            customer_arg.tracking_url_template, expected_tracking_url_template
        )
        self.assertEqual(
            customer_arg.final_url_suffix, expected_final_url_suffix
        )
