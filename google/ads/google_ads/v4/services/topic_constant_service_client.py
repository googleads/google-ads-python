...
"""Accesses the google.ads.googleads.v4.services TopicConstantService API."""
...
from google.ads.google_ads.v4.services import topic_constant_service_client_config
from google.ads.google_ads.v4.services.transports import topic_constant_service_grpc_transport
from google.ads.google_ads.v4.proto.services import topic_constant_service_pb2
...
class TopicConstantServiceClient(object):
    ...
    _INTERFACE_NAME = 'google.ads.googleads.v4.services.TopicConstantService'
        ...
        if transport:
            if callable(transport):
                self.transport = transport(
                    ...
                    default_class=topic_constant_service_grpc_transport.TopicConstantServiceGrpcTransport,
                )
            else:
                if credentials:
                    raise ValueError(
    ...
    def get_topic_constant(
        ...
        Returns:
            A :class:`~google.ads.googleads_v4.types.TopicConstant` instance.
        ...
        if 'get_topic_constant' not in self._inner_api_calls:
            self._inner_api_calls['get_topic_constant'] = google.api_core.gapic_v1.method.wrap_method(
                self.transport.get_topic_constant,
                default_retry=self._method_configs['GetTopicConstant'].retry,
                default_timeout=self._method_configs['GetTopicConstant'].timeout,
        ...
        return self._inner_api_calls['get_topic_constant'](request, retry=retry, timeout=timeout, metadata=metadata)
