...
"""Accesses the google.ads.googleads.v2.services FeedService API."""
...
from google.ads.google_ads.v2.services import feed_service_client_config
...
class FeedServiceClient(object):
    ...
    _INTERFACE_NAME = 'google.ads.googleads.v2.services.FeedService'
    ...
    def __init__(self, transport=None, channel=None, credentials=None,
        ...
        Args:
            transport (Union[~.FeedServiceGrpcTransport,
                    Callable[[~.Credentials, type], ~.FeedServiceGrpcTransport]): A transport
        ...
        if client_config is not None:
            ...
            client_config = feed_service_client_config.config
        ...
        if transport:
            if callable(transport):
                self.transport = transport(
                    ...
                    default_class=feed_service_grpc_transport.FeedServiceGrpcTransport,
                )
            else:
                if credentials:
                    raise ValueError(
            ...
            self.transport = feed_service_grpc_transport.FeedServiceGrpcTransport(
    ...
    def get_feed(
        ...
        Args:
            resource_name (str): The resource name of the feed to fetch.
        ...
        Returns:
            A :class:`~google.ads.googleads_v2.types.Feed` instance.
        ...
        request = feed_service_pb2.GetFeedRequest(
    ...
    def mutate_feeds(
        ...
        Args:
            customer_id (str): The ID of the customer whose feeds are being modified.
            operations (list[Union[dict, ~google.ads.googleads_v2.types.FeedOperation]]): The list of operations to perform on individual feeds.

                If a dict is provided, it must be of the same form as the protobuf
                message :class:`~google.ads.googleads_v2.types.FeedOperation`
        ...
        Returns:
            A :class:`~google.ads.googleads_v2.types.MutateFeedsResponse` instance.
        ...
        """
        # Wrap the transport method to add retry and timeout logic.
        if 'mutate_feeds' not in self._inner_api_calls:
            self._inner_api_calls['mutate_feeds'] = google.api_core.gapic_v1.method.wrap_method(
                self.transport.mutate_feeds,
                default_retry=self._method_configs['MutateFeeds'].retry,
                default_timeout=self._method_configs['MutateFeeds'].timeout,
                client_info=self._client_info,
            )

        request = feed_service_pb2.MutateFeedsRequest(
        ...
        return self._inner_api_calls['mutate_feeds'](request, retry=retry, timeout=timeout, metadata=metadata)
