...
"""Accesses the google.ads.googleads.v2.services LabelService API."""
...
from google.ads.google_ads.v2.services import label_service_client_config
...
class LabelServiceClient(object):
    ...
    _INTERFACE_NAME = 'google.ads.googleads.v2.services.LabelService'
    ...
    def __init__(self, transport=None, channel=None, credentials=None,
        ...
        Args:
            transport (Union[~.LabelServiceGrpcTransport,
                    Callable[[~.Credentials, type], ~.LabelServiceGrpcTransport]): A transport
        ...
        if client_config is not None:
            ...
            client_config = label_service_client_config.config
        ...
        if transport:
            if callable(transport):
                self.transport = transport(
                    ...
                    default_class=label_service_grpc_transport.LabelServiceGrpcTransport,
                )
            else:
                if credentials:
                    raise ValueError(
            ...
            self.transport = label_service_grpc_transport.LabelServiceGrpcTransport(
    ...
    def get_label(
        ...
        Args:
            resource_name (str): The resource name of the label to fetch.
        ...
        Returns:
            A :class:`~google.ads.googleads_v2.types.Label` instance.
        ...
        if 'get_label' not in self._inner_api_calls:
            self._inner_api_calls['get_label'] = google.api_core.gapic_v1.method.wrap_method(
                self.transport.get_label,
                default_retry=self._method_configs['GetLabel'].retry,
                default_timeout=self._method_configs['GetLabel'].timeout,
                client_info=self._client_info,
            )

        request = label_service_pb2.GetLabelRequest(
        ...
        return self._inner_api_calls['get_label'](request, retry=retry, timeout=timeout, metadata=metadata)
    ...
    def mutate_labels(
        ...
        Args:
            customer_id (str): ID of the customer whose labels are being modified.
            operations (list[Union[dict, ~google.ads.googleads_v2.types.LabelOperation]]): The list of operations to perform on labels.

                If a dict is provided, it must be of the same form as the protobuf
                message :class:`~google.ads.googleads_v2.types.LabelOperation`
        ...
        Returns:
            A :class:`~google.ads.googleads_v2.types.MutateLabelsResponse` instance.
        ...
        """
        # Wrap the transport method to add retry and timeout logic.
        if 'mutate_labels' not in self._inner_api_calls:
            self._inner_api_calls['mutate_labels'] = google.api_core.gapic_v1.method.wrap_method(
                self.transport.mutate_labels,
                default_retry=self._method_configs['MutateLabels'].retry,
                default_timeout=self._method_configs['MutateLabels'].timeout,
                client_info=self._client_info,
            )

        request = label_service_pb2.MutateLabelsRequest(
        ...
        return self._inner_api_calls['mutate_labels'](request, retry=retry, timeout=timeout, metadata=metadata)
