...
from google.ads.googleads.v19.services.services.customer_user_access_service import (
    CustomerUserAccessServiceClient,
)
from google.ads.googleads.v19.enums.types.access_role import AccessRoleEnum
from google.ads.googleads.v19.resources.types.customer_user_access import (
    CustomerUserAccess,
)
...
class TestUpdateUserAccess(unittest.TestCase):
    ...
    @patch(
        "examples.account_management.update_user_access.protobuf_helpers.field_mask"
    )
    @patch("examples.account_management.update_user_access.GoogleAdsClient")
    def test_main_updates_user_access(
        ...
        mock_client_instance.enums.AccessRoleEnum = (
            AccessRoleEnum.AccessRole
        )  # The inner enum with members
        ...
        mock_search_row.customer_user_access.email_address = (
            email_address_to_find
        )
        mock_search_row.customer_user_access.access_role = (
            original_access_role_enum
        )
        mock_search_row.customer_user_access.access_creation_date_time = (
            creation_date_time
        )
        ...
        expected_cua_resource_name = (
            f"customers/{customer_id}/customerUserAccesses/{user_id_to_return}"
        )
        mock_cua_service.customer_user_access_path.return_value = (
            expected_cua_resource_name
        )

        # Mock mutate_customer_user_access response
        mock_mutate_cua_response = MagicMock()
        mock_mutate_cua_response.result.resource_name = (
            expected_cua_resource_name  # Usually same after update
        )
        mock_cua_service.mutate_customer_user_access.return_value = (
            mock_mutate_cua_response
        )
        ...
        update_user_access.main(
            mock_client_instance,
            customer_id,
            email_address_to_find,
            access_role_str_to_set,
        )
        ...
        mock_ga_service.search.assert_called_once_with(
        ...
        self.assertEqual(
            mock_search_request.query.strip(), expected_search_query.strip()
        )

        # Assertions for modify_user_access (mutate call)
        mock_cua_service.customer_user_access_path.assert_called_once_with(
            customer_id, user_id_to_return
        )
        ...
        mutate_call_args = (
            mock_cua_service.mutate_customer_user_access.call_args
        )
        self.assertEqual(mutate_call_args.kwargs["customer_id"], customer_id)
        self.assertEqual(
            mutate_call_args.kwargs["operation"], mock_cua_operation
        )

        # Verify the updated object's attributes
        self.assertEqual(
            mock_cua_operation.update.resource_name, expected_cua_resource_name
        )
        self.assertEqual(
            mock_cua_operation.update.access_role, expected_new_access_role_enum
        )
