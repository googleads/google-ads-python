...
"""Accesses the google.ads.googleads.v2.services CustomInterestService API."""
...
from google.ads.google_ads.v2.services import custom_interest_service_client_config
...
class CustomInterestServiceClient(object):
    ...
    _INTERFACE_NAME = 'google.ads.googleads.v2.services.CustomInterestService'
    ...
    def __init__(self, transport=None, channel=None, credentials=None,
        ...
        Args:
            transport (Union[~.CustomInterestServiceGrpcTransport,
                    Callable[[~.Credentials, type], ~.CustomInterestServiceGrpcTransport]): A transport
        ...
        if client_config is not None:
            ...
            client_config = custom_interest_service_client_config.config
        ...
        if transport:
            if callable(transport):
                self.transport = transport(
                    ...
                    default_class=custom_interest_service_grpc_transport.CustomInterestServiceGrpcTransport,
                )
            else:
                if credentials:
                    raise ValueError(
            ...
            self.transport = custom_interest_service_grpc_transport.CustomInterestServiceGrpcTransport(
    ...
    def get_custom_interest(
        ...
        Args:
            resource_name (str): The resource name of the custom interest to fetch.
        ...
        Returns:
            A :class:`~google.ads.googleads_v2.types.CustomInterest` instance.
        ...
        if 'get_custom_interest' not in self._inner_api_calls:
            self._inner_api_calls['get_custom_interest'] = google.api_core.gapic_v1.method.wrap_method(
                self.transport.get_custom_interest,
                default_retry=self._method_configs['GetCustomInterest'].retry,
                default_timeout=self._method_configs['GetCustomInterest'].timeout,
                client_info=self._client_info,
            )

        request = custom_interest_service_pb2.GetCustomInterestRequest(
        ...
        return self._inner_api_calls['get_custom_interest'](request, retry=retry, timeout=timeout, metadata=metadata)
    ...
    def mutate_custom_interests(
        ...
        Args:
            customer_id (str): The ID of the customer whose custom interests are being modified.
            operations (list[Union[dict, ~google.ads.googleads_v2.types.CustomInterestOperation]]): The list of operations to perform on individual custom interests.

                If a dict is provided, it must be of the same form as the protobuf
                message :class:`~google.ads.googleads_v2.types.CustomInterestOperation`
        ...
        Returns:
            A :class:`~google.ads.googleads_v2.types.MutateCustomInterestsResponse` instance.
        ...
        """
        # Wrap the transport method to add retry and timeout logic.
        if 'mutate_custom_interests' not in self._inner_api_calls:
            self._inner_api_calls['mutate_custom_interests'] = google.api_core.gapic_v1.method.wrap_method(
                self.transport.mutate_custom_interests,
                default_retry=self._method_configs['MutateCustomInterests'].retry,
                default_timeout=self._method_configs['MutateCustomInterests'].timeout,
                client_info=self._client_info,
            )

        request = custom_interest_service_pb2.MutateCustomInterestsRequest(
        ...
        return self._inner_api_calls['mutate_custom_interests'](request, retry=retry, timeout=timeout, metadata=metadata)
