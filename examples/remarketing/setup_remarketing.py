...
def _create_user_list(client, customer_id):
    ...
    rule_item_group = (
        user_list.rule_based_user_list.expression_rule_user_list.rule.rule_item_groups.add()
    )
    rule_item = rule_item_group.rule_items.add()
    # Uses a built-in parameter to create a domain URL rule.
    rule_item.name = "url__"
    rule_item.string_rule_item.operator = client.get_type(
        "UserListStringRuleItemOperatorEnum", version="v6"
    ).CONTAINS
    rule_item.string_rule_item.value = "example.com"
...
def _modify_ad_group_bids(
    ...
    field_mask = protobuf_helpers.field_mask(None, ad_group_criterion)
...
def _remove_existing_criteria_from_ad_group(client, customer_id, campaign_id):
    ...
    for ad_group_criterion_resource_name in all_ad_group_criteria:
        remove_operation = client.get_type(
            "AdGroupCriterionOperation", version="v6"
        )
...
def _modify_campaign_bids(
    ...
    field_mask = protobuf_helpers.field_mask(None, campaign_criterion)
    campaign_criterion_operation.update_mask.CopyFrom(field_mask)
