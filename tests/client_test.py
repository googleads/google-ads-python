...
class LoggingInterceptorTest(TestCase):
    ...
    def _create_test_interceptor(self, config=_MOCK_CONFIG,
        ...
        """Creates a LoggingInterceptor instance. Accepts parameters that are
        used to override defaults when needed for testing.
    ...
    def _get_mock_exception(self):
        """Generates a mock GoogleAdsException exception instance for use in
        tests.
    ...
    def _get_mock_transport_exception(self):
        """Generates a mock transport error, i.e. an error not generated by the
        Google Ads API that is not an instance of GoogleAdsException.
    ...
    def _get_mock_response(self, failed=False):
        """Generates a mock response object for use in tests. Accepts a "failed"
        param that tells the returned mocked response to mimic a failed
        response.
    ...
    def _get_mock_continuation_fn(self, fail=False):
        """Generates a mock continuation function for use in tests. Accepts a
        "failed" param that tell the function to return a failed mock response
        or not.
    ...
    def test_init_no_config(self):
        """LoggingInterceptor should not call logging.dictConfig if it is not
    ...
    def test_init_with_config(self):
        """LoggingInterceptor should call logging.dictConfig if it is
        initialized with a configuration.
        """
        config = {'test': True}
        with mock.patch('logging.config.dictConfig') as mock_dictConfig:
    ...
    def test_intercept_unary_unary_successful_request(self):
        """LoggingInterceptor should call logging.info and logging.debug with
    ...
    def test_intercept_unary_unary_failed_request(self):
        """LoggingInterceptor should call logging.warning and logging.info with
    ...
    def test_get_initial_metadata(self):
        """_get_initial_metatada returns a tuple of metadata from the
        client_call_details passed into the intercept_unary_unary method.
        """
        with mock.patch('logging.config.dictConfig'):
            mock_client_call_details = mock.Mock()
            mock_client_call_details.metadata = self._MOCK_INITIAL_METADATA
    ...
    def test_get_initial_metadata_none(self):
        """_get_initial_metatada returns an empty tuple if it cannot retrieve
        initial_metadata from client_call_details.
        """
        with mock.patch('logging.config.dictConfig'):
            mock_client_call_details = {}
            interceptor = self._create_test_interceptor()
    ...
    def test_get_call_method(self):
        """_get_call_method returns a str of the call method from the
        client_call_details passed into the intercept_unary_unary method.
        """
        with mock.patch('logging.config.dictConfig'):
            mock_client_call_details = mock.Mock()
    ...
    def test_get_call_method_none(self):
        """_get_call_method returns None if it cannot retrieve the method from
        client_call_details.
        """
        with mock.patch('logging.config.dictConfig'):
            mock_client_call_details = {}
    ...
    def test_get_request_id(self):
        """_get_request_id returns a request ID str from a response object
        returned by the continuation fuction passed into the
        intercept_unary_unary method.
        """
        with mock.patch('logging.config.dictConfig'):
    ...
    def test_get_request_id_google_ads_failure(self):
        """_get_request_id returns a request ID str from an exception object if
        it is a GoogleAdsException.
        """
        with mock.patch('logging.config.dictConfig'):
    ...
    def test_get_request_id_transport_failure(self):
        """_get_request_id returns None if a transport exception without a
        request_id is passed in.
        """
        with mock.patch('logging.config.dictConfig'):
            mock_response = self._get_mock_response(failed=True)
            mock_exception = mock_response.exception()
    ...
    def test_parse_response_to_json(self):
        """_parse_response_to_json passes a successful response message to the
    ...
    def test_parse_response_to_json_google_ads_failure(self):
        """_parse_response_to_json passes the failure message of a
        GoogleAdsException to the MessageToJson function.
        """
        with mock.patch('logging.config.dictConfig'), \
            mock.patch(
                'google.ads.google_ads.client.MessageToJson') as mock_formatter:
    ...
    def test_parse_response_to_json_transport_failure(self):
        """_parse_response_to_json passes the return value of a transport
        error's debug_error_string to the _parse_to_json method.
        """
        with mock.patch('logging.config.dictConfig'), \
            mock.patch(
    ...
    def test_parse_response_to_json_unknown_failure(self):
        """_parse_response_to_json returns an empty JSON string if the response
        and exception cannot be parsed to JSON.
        """
        with mock.patch('logging.config.dictConfig'):
            mock_response = mock.Mock()
    ...
    def test_get_trailing_metadata(self):
        """_get_trailing_metadata retrieves metadata from a response object."""
        with mock.patch('logging.config.dictConfig'):
            mock_response = self._get_mock_response()
            mock_exception = mock_response.exception()
            interceptor = self._create_test_interceptor()
    ...
    def test_get_trailing_metadata_google_ads_failure(self):
        """_get_trailing_metadata retrieves metadata from a failed response."""
        with mock.patch('logging.config.dictConfig'):
            mock_response = self._get_mock_response(failed=True)
            mock_exception = mock_response.exception()
            interceptor = self._create_test_interceptor()
    ...
    def test_get_trailing_metadata_transport_failure(self):
        """_get_trailing_metadata retrieves metadata from a transport error."""
        with mock.patch('logging.config.dictConfig'):
            mock_response = mock.Mock()
            mock_exception = self._get_mock_transport_exception()
            interceptor = self._create_test_interceptor()
    ...
    def test_get_trailing_metadata_unknown_failure(self):
        """_get_trailing_metadata returns an empty tuple if metadata cannot be
        found on the given response or exception.
        """
        with mock.patch('logging.config.dictConfig'):
            mock_response = {}
            mock_exception = self._get_mock_transport_exception()
    ...
    def test_get_fault_message(self):
        """_get_fault_message returns None if an error message cannot be found
    ...
    def test_get_fault_message_google_ads_failure(self):
        """_get_fault_message retrieves an error message from a
        GoogleAdsException.
        """
        with mock.patch('logging.config.dictConfig'):
            mock_exception = self._get_mock_exception()
    ...
    def test_get_fault_message_transport_failure(self):
        """_get_fault_message retrieves an error message from a transport error
        object.
        """
        with mock.patch('logging.config.dictConfig'):
            mock_exception = self._get_mock_transport_exception()
...
class ExceptionInterceptorTest(TestCase):
    ...
    def test_get_request_id_no_id(self):
        """_get_request_id returns None if the given metadata tuple does not
    ...
    def test_get_google_ads_failure(self):
        """_get_google_ads_failure obtains the content of a google ads failure
        from trailing_metadata, serializes it into a GoogleAdsFailure instance
        and returns it.
        """
        interceptor = self._create_test_interceptor()
    ...
    def test_get_google_ads_failure_decode_error(self):
        """If the given metadata tuple has an item with the googleads failure
        key, but the value generates an error when decoded,
        _get_google_ads_failure returns None.
        """
        interceptor = self._create_test_interceptor()
    ...
    def test_get_google_ads_failure_no_failure_key(self):
        """_get_google_ads_failure returns None if the given metadata tuple does
        not have an item containing the googleads failure key.
        """
        mock_metadata = (('another-key', 'another-val'),)
        interceptor = self._create_test_interceptor()
        result = interceptor._get_google_ads_failure(mock_metadata)
    ...
    def test_get_google_ads_failure_with_None(self):
        """_get_google_ads_failure returns None if None is passed."""
        interceptor = self._create_test_interceptor()
