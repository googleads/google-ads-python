...
"""Accesses the google.ads.googleads.v4.services BillingSetupService API."""
...
from google.ads.google_ads.v4.services import billing_setup_service_client_config
from google.ads.google_ads.v4.services.transports import billing_setup_service_grpc_transport
from google.ads.google_ads.v4.proto.services import billing_setup_service_pb2
...
class BillingSetupServiceClient(object):
    ...
    _INTERFACE_NAME = 'google.ads.googleads.v4.services.BillingSetupService'
        ...
        if transport:
            if callable(transport):
                self.transport = transport(
                    ...
                    default_class=billing_setup_service_grpc_transport.BillingSetupServiceGrpcTransport,
                )
            else:
                if credentials:
                    raise ValueError(
    ...
    def get_billing_setup(
        ...
        Returns:
            A :class:`~google.ads.googleads_v4.types.BillingSetup` instance.
        ...
        if 'get_billing_setup' not in self._inner_api_calls:
            self._inner_api_calls['get_billing_setup'] = google.api_core.gapic_v1.method.wrap_method(
                self.transport.get_billing_setup,
                default_retry=self._method_configs['GetBillingSetup'].retry,
                default_timeout=self._method_configs['GetBillingSetup'].timeout,
        ...
        return self._inner_api_calls['get_billing_setup'](request, retry=retry, timeout=timeout, metadata=metadata)
    ...
    def mutate_billing_setup(
        ...
        Args:
            ...
            operation_ (Union[dict, ~google.ads.googleads_v4.types.BillingSetupOperation]): Required. The operation to perform.

                If a dict is provided, it must be of the same form as the protobuf
                message :class:`~google.ads.googleads_v4.types.BillingSetupOperation`
            retry (Optional[google.api_core.retry.Retry]):  A retry object used
                to retry requests. If ``None`` is specified, requests will not
                be retried.
            timeout (Optional[float]): The amount of time, in seconds, to wait
        ...
        Returns:
            A :class:`~google.ads.googleads_v4.types.MutateBillingSetupResponse` instance.
        ...
        if 'mutate_billing_setup' not in self._inner_api_calls:
            self._inner_api_calls['mutate_billing_setup'] = google.api_core.gapic_v1.method.wrap_method(
                self.transport.mutate_billing_setup,
                default_retry=self._method_configs['MutateBillingSetup'].retry,
                default_timeout=self._method_configs['MutateBillingSetup'].timeout,
                client_info=self._client_info,
            )
        ...
        return self._inner_api_calls['mutate_billing_setup'](request, retry=retry, timeout=timeout, metadata=metadata)
