...
def main(client, customer_id):
    ...
    checkout_and_cart_size_rule_group = (
        expression_rule_user_list_info.rule.rule_item_groups.add()
    )

    # Creates a rule targeting any user that visited the checkout page.
    checkout_rule = checkout_and_cart_size_rule_group.rule_items.add()
    ...
    # cart.
    cart_size_rule = checkout_and_cart_size_rule_group.rule_items.add()
    # The rule variable name must match a corresponding key name fired from a
    # pixel.
    ...
    cart_size_number_rule_item.operator = client.get_type(
        "UserListNumberRuleItemOperatorEnum", version="v6"
    ).GREATER_THAN
    ...
    start_date_rule = checkout_date_rule_group.rule_items.add()
    ...
    start_date_rule_item.operator = client.get_type(
        "UserListDateRuleItemOperatorEnum", version="v6"
    ).AFTER
    start_date_rule_item.value = "20191031"

    # Creates the rule item for checkout end date.
    end_date_rule = checkout_date_rule_group.rule_items.add()
    end_date_rule.name = "checkoutdate"
    end_date_rule_item = end_date_rule.date_rule_item
    end_date_rule_item.operator = client.get_type(
        "UserListDateRuleItemOperatorEnum", version="v6"
    ).BEFORE
