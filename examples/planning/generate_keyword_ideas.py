...
def main(client, customer_id, location_ids, language_id, keyword_texts,
         page_url):
    keyword_plan_idea_service = client.get_service('KeywordPlanIdeaService',
                                                   version='v4')
    keyword_competition_level_enum = client.get_type(
        'KeywordPlanCompetitionLevelEnum',
            version='v4').KeywordPlanCompetitionLevel
    keyword_plan_network = client.get_type(
        'KeywordPlanNetworkEnum', version='v4').GOOGLE_SEARCH_AND_PARTNERS
    ...
    if not keyword_texts and page_url:
        url_seed = client.get_type('UrlSeed', version='v4')
    ...
    if keyword_texts and not page_url:
        keyword_seed = client.get_type('KeywordSeed', version='v4')
    ...
    if keyword_texts and page_url:
        keyword_url_seed = client.get_type('KeywordAndUrlSeed', version='v4')
    ...
    try:
        keyword_ideas = keyword_plan_idea_service.generate_keyword_ideas(
            customer_id, language, locations, False, keyword_plan_network,
...
def map_locations_to_string_values(client, location_ids):
    ...
    for location_id in location_ids:
        location = client.get_type('StringValue', version='v4')
...
def map_language_to_string_value(client, language_id):
    ...
    language.value = client.get_service('LanguageConstantService',
                                        version='v4').language_constant_path(
                                            language_id)
    return language
...
if __name__ == '__main__':
    ...
    parser.add_argument('-k', '--keyword_texts', nargs='+', type=str,
                        required=False, default=[],
                        help='Space-delimited list of starter keywords')
    # To determine the appropriate location IDs, see:
    # https://developers.google.com/adwords/api/docs/appendix/geotargeting.
    parser.add_argument('-l', '--location_ids', nargs='+', type=str,
                        required=False, default=_DEFAULT_LOCATION_IDS,
    ...
    parser.add_argument('-i', '--language_id', type=str,
                        required=False, default=_DEFAULT_LANGUAGE_ID,
    ...
    main(google_ads_client, args.customer_id, args.location_ids,
         args.language_id, args.keyword_texts, args.page_url)
