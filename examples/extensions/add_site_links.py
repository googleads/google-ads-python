...
def main(client, customer_id, campaign_id):
    ...
    campaign_ext_setting_service = client.get_service(
        'CampaignExtensionSettingService', version='v2')
    ...
    campaign_ext_setting_operation = client.get_type(
        'CampaignExtensionSettingOperation', version='v2')
    extension_type_enum = client.get_type(
        'ExtensionTypeEnum', version='v2')
...
def _create_extension_feed_items(client, customer_id, campaign_resource_name):
    extension_feed_item_service = client.get_service(
        'ExtensionFeedItemService', version='v2')
    ...
    extension_feed_item_operation1 = client.get_type(
        'ExtensionFeedItemOperation', version='v2')
    ...
    _populate_ad_schedule(
        extension_feed_item4.ad_schedules.add(), day_of_week_enum.MONDAY, 18,
        minute_of_hour_enum.ZERO, 21, minute_of_hour_enum.ZERO)
    _populate_ad_schedule(
        extension_feed_item4.ad_schedules.add(), day_of_week_enum.TUESDAY, 18,
        minute_of_hour_enum.ZERO, 21, minute_of_hour_enum.ZERO)
    _populate_ad_schedule(
        extension_feed_item4.ad_schedules.add(), day_of_week_enum.WEDNESDAY, 18,
        minute_of_hour_enum.ZERO, 21, minute_of_hour_enum.ZERO)
    _populate_ad_schedule(
        extension_feed_item4.ad_schedules.add(), day_of_week_enum.THURSDAY, 18,
        minute_of_hour_enum.ZERO, 21, minute_of_hour_enum.ZERO)
    _populate_ad_schedule(
        extension_feed_item4.ad_schedules.add(), day_of_week_enum.FRIDAY, 18,
        minute_of_hour_enum.ZERO, 21, minute_of_hour_enum.ZERO)
    ...
    return [_to_string_value(client, resource_name) for resource_name in
            [result.resource_name for result in feed_response.results]]
...
def _get_thanksgiving_string_date_range():
    ...
    return _DateRange(start_dt.strftime(_DT_FMT), end_dt.strftime(_DT_FMT))
